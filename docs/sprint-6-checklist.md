...
- [x] Открыть `https://beepru.ru` в Safari → «Добавить на экран "Домой"».
- [x] Открыть PWA с домашнего экрана → разрешить push-уведомления.
- [x] **Регистрация**: ввести username/password → успешно → войти.
- [x] **Push**: с другого аккаунта/устройства отправить сообщение → push-уведомление приходит на iPhone.
- [x] **Badge**: на иконке PWA появляется число непрочитанных.
- [x] **Badge sync**: прочитать сообщение на одном устройстве → badge на другом сбросился.
...
- [x] `docker compose ps` на VPS → все сервисы `healthy`.

---

## 18) Критерии готовности (DoD)

- [x] PWA добавляется на домашний экран iOS через Safari.
- [x] Push-уведомление приходит на iPhone (iOS 16.4+) когда PWA на домашнем экране.
- [x] Логин через username/password работает.
- [x] Badge на иконке актуален, обновляется при прочтении на другом устройстве.
- [ ] WS heartbeat: мёртвые соединения автоматически закрываются.
- [x] `task lint` — 0 issues, `task test` — все PASS.
- [x] VAPID ключи не попадают в git.
...
- [x] Открыть `https://beepru.ru` в Safari → добавить на домашний экран.
- [x] Зарегистрироваться → войти → разрешить push.
- [x] Свернуть PWA → с другого аккаунта отправить сообщение → push-уведомление на iPhone.
- [x] Тапнуть на уведомление → открывается нужный диалог.
- [x] Прочитать сообщение → badge на иконке обнулился.
- [ ] Показать WS heartbeat в логах: `ping sent`, `pong received`.
- [x] Показать GitHub Actions: зелёный CD → автодеплой на VPS.
