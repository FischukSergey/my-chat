...
## 9) WebSocket-события Sprint 2 (полный список)

В дополнение к событиям Sprint 1 (`message_new`, `message_delivered`, `message_read`) добавляется:

### `badge_updated`

Отправляется читателю после успешного `POST /api/v1/messages/{id}/read`.
...
Правила:
- `badge == unread_count` в Sprint 2.
- Событие отправляется читателю (тому, кто вызвал `read`), а не отправителю.
- Если посчитать `unread_count` не удалось (ошибка БД) — событие не отправляется; основная операция `MarkRead` при этом не откатывается.

## 10) Критерий совместимости
